...
export default function Home({ articles, news }) {
  ...
  return (
    <Layout>
        ...
        <div className="section-inner">
          ...
          {latestNews.length === 0 ? (
            <div className="text-center py-8 reveal">
              <span className="inline-block text-2xl font-bold text-matte-gold/50" style={{ fontFamily: 'var(--font-serif)' }}>قريباً</span>
              <p className="text-charcoal/40 text-sm mt-2">سيتم نشر أخبار وإنجازات المؤسسة قريباً</p>
            </div>
          ) : (
            <div className="experience-grid">
              {latestNews.map((item) => (
      ...
      {/* Map - Static image with elegant Google Maps button */}
      <section className="map-section" aria-label="موقع المكتب">
        <div className="map-container">
          <Image
            src="/map-static.webp"
            alt="موقع مؤسسة الأستاذ محمود عبد الحميد للمحاماة في أسوان"
            width={1200}
            height={675}
            quality={85}
            priority
          ...
          <div className="map-overlay">
            <a
              href="https://maps.app.goo.gl/inYTfr9Js78JwXqx5"
              target="_blank"
              rel="noopener noreferrer"
              className="map-btn"
              aria-label="افتح الموقع في خرائط جوجل"
            >
              <span className="map-btn-icon">
