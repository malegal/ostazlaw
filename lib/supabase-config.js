...
// استعلام موحد يعيد قضية قضائية أو ملف خدمة مهنية حسب الكود ورقم الهاتف.
export async function getPortalFileData(phone, code) {
  const digits = String(phone || '').replace(/\D/g, '');
  const localPhone = digits.replace(/^20/, '').replace(/^0/, '');
  const phoneVariants = [...new Set([digits, `20${localPhone}`, `0${localPhone}`].filter(Boolean))];
  let lastError = null;
  ...
  for (const phoneVariant of phoneVariants) {
    const { data, error } = await supabase.rpc('get_portal_file_data', {
      p_phone: phoneVariant,
      p_code: code,
    });
    if (error) lastError = error;
    if (data && data.file) return { data, error: null };
  }
